...
class PlantsDataloader:
    def __init__(self, df, feature_extractor, device, batch_size=64):
        self.device = device
        self.df = df
        self.damage_types = [
            "healthy",
...
def main():
    ...
    train_df, val_df = train_test_split(both_df, test_size=0.1, random_state=seed)

    feature_extractor = get_feature_extractor(model_id, device)
    train_dataloader = PlantsDataloader(
        train_df, feature_extractor, device, batch_size=batch_size
    )
    val_dataloader = PlantsDataloader(
        val_df, feature_extractor, device, batch_size=batch_size
    )

    model = nn.Linear(768, 5).to(device)
        ...
        for i in range(5):
            ...
            for threshold in thresholds:
                ...
                if true_positive.sum() == 0:
                    ...
                    recall.append(
                        (
                            (preds == all_labels[:, i])[true_positive].int().sum(0)
                            / true_positive.sum()
                        )
                        .cpu()
                        .item()
            ...
            # fpr_curve, tpr_curve, _ = sklearn.metrics.roc_curve(
            #     all_labels_np[:, i], all_predictions_np[:, i]
            # )
            # roc_aucs.append(sklearn.metrics.auc(fpr_curve, tpr_curve))
